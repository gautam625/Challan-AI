        ...
        if len(plates) == 0:
            ...
            with col1:
                    if st.button("🚨 Issue Challan"):
                        insert_challan(car_number=car_number,amount=fine,reason=reason,date=str(datetime.now().date()), status="Pending")
                        st.success("✅ Challan Issued Successfully")

            # RIGHT BUTTON (Pay Challan)
            with col2:
                    if st.button("💳 Pay Challan"):
                        insert_challan(car_number=car_number,amount=fine,reason=reason,date=str(datetime.now().date()), status="Paid")
            ...
            # WhatsApp
            with colA:
                if owner and st.button("📞 Send WhatsApp"):
                    send_msg(owner,car_number,reason,fine,total_pending,grand_total,date,time)
            ...
            with colB:
                 if owner:
                    ...
                    st.download_button(label="📄 Download PDF",data=pdf_bytes,file_name=f"{car_number}_challan.pdf",mime="application/pdf")
                    os.remove(temp_path)
